// Initial Stable Firebase Configuration Matrix
    ...
    if (user) {
        ...
        db.collection('users').doc(user.uid).onSnapshot((doc) => {
            if (doc.exists) {
                ...
                document.getElementById('modal-user-coins').innerText = currentUserData.coins || 0;
...
function switchMatchTab(tabName) {
    ...
    document.getElementById(`tab-${tabName}`).style.background = '#ff4655';
...
function renderMatchesList() {
    ...
    activeMatches.forEach(t => {
        ...
        if (currentActiveTab === "my_joined") {
            // Evaluated inside snapshot listener
        ...
        db.collection('tournaments').doc(uniqueMatchKey).onSnapshot((doc) => {
            ...
            document.getElementById(`count_${uniqueMatchKey}`).innerText = `👥 Joined: ${joinedCount}/${t.maxSlots}`;
            ...
            if (status === "past") {
                ...
                if (isUserJoined) {
                    ...
                    lockWrapper.classList.add('hidden');
                } else {
                    tableWrapper.classList.add('hidden');
                    lockWrapper.classList.remove('hidden');
                }
        ...
        syncDynamicMatchFees(uniqueMatchKey, t.fee || 10);
    });
